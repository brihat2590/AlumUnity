...
import { Calendar, Clock, MapPin, Plus, Video, Sparkles, ArrowUpRight, Users } from 'lucide-react';
    ...
    return (
      <Dialog>
        <DialogTrigger asChild>
          <Button
            variant="ghost"
            className="h-9 rounded-full border border-[#4647d3]/20 bg-[#4647d3]/5 px-4 text-xs font-semibold text-[#4647d3] transition-all hover:bg-[#4647d3] hover:text-white"
          >
        ...
        <header className="mb-10 sm:mb-14">
          <div className="flex flex-col gap-6 sm:flex-row sm:items-end sm:justify-between">
            <div className="space-y-2">
              <div className="inline-flex items-center gap-2 rounded-full border border-[#4647d3]/20 bg-[#4647d3]/5 px-3 py-1">
                <Sparkles className="h-3.5 w-3.5 text-[#4647d3]" />
                <span className="text-[11px] font-semibold uppercase tracking-widest text-[#4647d3]">Upcoming Events</span>
              </div>
              <h1
                className="text-3xl font-extrabold tracking-tight text-[#1a1a2e] sm:text-4xl md:text-5xl"
                style={{ fontFamily: 'var(--font-manrope)' }}
              >
                Events & <span className="bg-gradient-to-r from-[#4647d3] to-[#7c3aed] bg-clip-text text-transparent">Meetups</span>
              </h1>
              <p
                className="max-w-lg text-sm leading-relaxed text-slate-500"
                style={{ fontFamily: 'var(--font-inter)' }}
              >
                Discover and join upcoming events curated for alumni and students. Connect, learn, and grow together.
              </p>
            </div>

            <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
              <DialogTrigger asChild>
                <Button className="h-11 gap-2 rounded-full bg-[#4647d3] px-5 text-sm font-semibold text-white shadow-lg shadow-[#4647d3]/25 transition-all hover:bg-[#3c3db8] hover:shadow-xl hover:shadow-[#4647d3]/30">
              ...
              <DialogContent className="max-w-[calc(100vw-2rem)] rounded-2xl border border-slate-100 bg-white p-0 shadow-xl sm:max-w-[520px]">
                <div className="border-b border-slate-100 bg-gradient-to-r from-[#4647d3]/5 to-transparent px-6 py-5">
                  <DialogHeader>
                  ...
                  <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                    <div className="space-y-2">
                      ...
                      <Input
                        ...
                        onChange={handleChange}
                        className="h-11 rounded-xl border-slate-200 bg-slate-50 text-sm transition-all focus:border-[#4647d3]/30 focus:bg-white focus:ring-2 focus:ring-[#4647d3]/10 px-3"
                      />
                    </div>
                    <div className="space-y-2">
                ...
                <div className="flex items-center justify-end gap-3 border-t border-slate-100 bg-slate-50/50 px-6 py-4">
                  ...
                  <Button
                    ...
                    className="h-10 rounded-full bg-[#4647d3] px-6 text-sm font-semibold text-white shadow-md shadow-[#4647d3]/20 transition-all hover:bg-[#3c3db8]"
                  >
                    Publish Event
                  </Button>
                </div>
        ...
        <section>
          {events.length > 0 ? (
            <div className="grid grid-cols-1 gap-5 md:grid-cols-2 lg:grid-cols-3">
              {events.map((event: any) => {
                ...
                return (
                  <article
                    key={event.id}
                    className="group relative flex flex-col overflow-hidden rounded-2xl border border-slate-100 bg-white p-5 shadow-sm transition-all duration-300 hover:-translate-y-1 hover:border-[#4647d3]/20 hover:shadow-lg"
                  >
                    <div className="mb-4 flex items-start justify-between">
                      <div className="flex items-center gap-3">
                        <div className="flex h-14 w-14 flex-col items-center justify-center rounded-xl bg-gradient-to-br from-[#4647d3] to-[#7c3aed] text-white shadow-md shadow-[#4647d3]/20">
                    ...
                    <div className="space-y-2.5 border-t border-slate-50 pt-4">
                      {event.location && (
                        <div className="flex items-center gap-2 text-sm text-slate-600">
                          <div className="flex h-7 w-7 items-center justify-center rounded-full bg-[#4647d3]/10">
                      ...
                      {event.meet_link && (
                        <div className="flex items-center justify-between">
                          <div className="flex items-center gap-2 text-sm text-slate-600">
                            <div className="flex h-7 w-7 items-center justify-center rounded-full bg-[#7c3aed]/10">
            ...
            <div className="flex flex-col items-center justify-center rounded-2xl border border-dashed border-slate-200 bg-white px-6 py-20 text-center">
              <div className="mb-4 flex h-20 w-20 items-center justify-center rounded-2xl bg-gradient-to-br from-[#4647d3]/10 to-[#7c3aed]/10">
              ...
              <p
                ...
                style={{ fontFamily: 'var(--font-inter)' }}
              >
                There are no upcoming events at the moment. Be the first to create one and bring the community together.
              </p>
              <Button
                onClick={() => setIsDialogOpen(true)}
                className="h-11 gap-2 rounded-full bg-[#4647d3] px-6 text-sm font-semibold text-white shadow-lg shadow-[#4647d3]/25 transition-all hover:bg-[#3c3db8] hover:shadow-xl hover:shadow-[#4647d3]/30"
              >
                <Plus className="h-4 w-4" />
                Create First Event
              </Button>
