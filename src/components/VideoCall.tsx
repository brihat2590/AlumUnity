...
export default function VideoCall({ roomId, mode = "video" }: Props) {
  ...
  return (
    <div className="flex min-h-screen flex-col items-center justify-center bg-slate-50 p-4 text-slate-900">
      ...
      <div className="mt-6 flex w-full max-w-5xl flex-wrap items-center justify-center gap-2 sm:gap-3">
        <button
          type="button"
          onClick={() => setIsMuted((current) => !current)}
          className="rounded-full border border-slate-200 bg-white px-3 sm:px-4 py-2 text-xs sm:text-sm text-slate-700 transition-colors hover:bg-slate-50"
        >
          {isMuted ? "Unmute" : "Mute"}
        </button>
        <button
          ...
          disabled={mode === "voice"}
          className="rounded-full border border-slate-200 bg-white px-3 sm:px-4 py-2 text-xs sm:text-sm text-slate-700 transition-colors hover:bg-slate-50 disabled:cursor-not-allowed disabled:opacity-50"
        >
        ...
        <button
          type="button"
          onClick={hangUp}
          className="rounded-full bg-red-500 px-3 sm:px-4 py-2 text-xs sm:text-sm font-semibold text-white transition-colors hover:bg-red-600"
        >
          End call
        </button>
      </div>
